...
def convert_blockgr_to_race(indic=None):
  ...
  f2 = open("state_p18_sr_blk_map.csv", "r")
  outf1 = open("srprec_to_race.csv","w")
  outf2 = open("srprec_to_race_dic.csv","w")
    ...
    for line in reader(f1):
      ...
      if len(racelines) == len(block_races):
        ...
        for pct in reader(f2):
          if blockgr in pct[5][:12]:
            if pct[4] not in count:
              count[pct[4]] = [ 1.0/39 * float(pct[-1]) * race_breakdown[i] for i in xrange(len(race_breakdown)) ]
            else:
              count[pct[4]] = [ count[pct[4]][i] + 1.0/39 * float(pct[-1]) * race_breakdown[i] for i in xrange(len(race_breakdown)) ]
        racelines=[]
  else:
    count = {}
  for key in count:
